...
public interface ParentPlatformMapper {
    ...
    @Select("select 'channel' as name, count(pgc.platformId) count from platform_gb_channel pgc left join device_channel dc on dc.id = pgc.deviceChannelId where  pgc.platformId=#{platformId} and dc.channel_id =#{gbId} " +
            "union " +
